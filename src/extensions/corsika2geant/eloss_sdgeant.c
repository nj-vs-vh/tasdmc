...
int get_elosses(int corid, double ke, double sectheta, double *elossUpper, double *elossLower)
{
  ...
  if (isecthetalo < isecthetaup)
    isectheta = ((drand48() < (sectheta - par->secthetamin - ((double) isecthetalo + 0.5) * par->secthetabs)
        / par->secthetabs) ? isecthetaup : isecthetalo);
  ...
  peloss = par->ke[ike].peloss[isectheta] + par->ke[ikelo].peloss_kepc[isectheta] * (log10ke - log10ke_bc)
      + par->ke[ike].peloss_sepc[isecthetalo] * (sectheta - sectheta_bc);
