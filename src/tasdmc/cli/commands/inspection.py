...
@click.option("-p", "--page", "pagesize", default=1, help="Page size; 0 for no pagination")
@click.option("-a", "--all", is_flag=True, default=False, help="Inspect all pipelines, not only failed ones")
@loading_run_by_name
@error_catching
def inspect_cmd(pagesize: int, verbose: bool, all: bool):
    if config.is_distributed_run():
        click.echo("Not available for distributed run, please inspect your nodes manually")
        return
    pipeline_ids = fileio.get_all_pipeline_ids() if all else fileio.get_failed_pipeline_ids()
