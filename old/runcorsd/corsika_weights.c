		  ...
		  float *mo)		
{
  ...
  *dist = normalizef(d1, d1);
  ...
  EO[1] = buf[5];
  EO[2] = Z0;
  for ( i = 0; i < 3; i++ ) EO[i] = origin[i] - EO[i];
  pEO = dotf(R1, EO); 
  mEO = normalizef(EO, EO);
  tc = buf[6]*CSPEED;
  *R = (tc*tc - mEO*mEO)/(tc + pEO)/2.0;
  Rtmp = (((float)h2mo(Z0, ATMOS_MODEL)-
	   (float)h2mo(origin[2], ATMOS_MODEL))/EO[2])-(30.*hadgen);
  Rtmp = ((float)mo2h(((float)h2mo(Z0,ATMOS_MODEL)-
		       Rtmp*EO[2]), ATMOS_MODEL)-Z0)/EO[2];
  if ( Rtmp > 0 && *R > Rtmp) *R = Rtmp;
  *mo = ((float)h2mo(Z0, ATMOS_MODEL)-
	 (float)h2mo((-R1[2]*(*R)+Z0), ATMOS_MODEL))*sectheta;
  return;
}
...
void
polar_gaussf(float pair[], float trunc)
{
...
void
crossf(float v1[], float v2[], float v3[]) {
